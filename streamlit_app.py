...
# ========== MODELING ==========
    ...
    if 'df_musim' in st.session_state:
        ...
        if st.button("🚀 Mulai Training (dengan Scheduler & Checkpoint)"):
            ...
            # ------------------------- MODEL 2 (LSTM Deep) -------------------------
        st.subheader("🧠 Model LSTM 2 (Arsitektur Lebih Dalam)")
        ...
        with st.expander("📋 LSTM Model 2 Summary"):
            st.text(model2.summary())

        # Fungsi pelatihan (boleh pakai ulang yang sebelumnya jika sudah ada)
        def train_model_streamlit(model, X_train, y_train, X_test, y_test,
                                  learning_rate=0.001, batch_size=32, epochs=50,
                                  patience=5, filepath='best_model2.h5'):
            ...
            with st.spinner("⏳ Training Model 2 sedang berlangsung..."):
                history = model.fit(X_train, y_train,
                                    epochs=epochs,
                                    batch_size=batch_size,
                                    validation_data=(X_test, y_test),
            ...
            st.success("✅ Model 2 selesai dilatih!")

            # Evaluasi
            loss, mae = model.evaluate(X_test, y_test, verbose=0)
            st.metric("MSE (Model 2)", f"{loss:.5f}")
            st.metric("MAE (Model 2)", f"{mae:.5f}")
            ...
            ax2.set_ylabel('Loss')
            ax2.set_title('Model 2 - Training History')
            ax2.legend()
            st.pyplot(fig2)

            return history, loss

        # Tombol pelatihan untuk Model 2
        if st.button("🚀 Latih Model 2 (LSTM Deep)"):
